...
export function createLeafletViewer(opts = {}) {
  ...
  const viewManager = createLeafletViewManager({
    ...
    getCanvasMaxZoom: (id) => api.getCanvasMaxZoom(id),
  ...
  function ensurePanes() {
    ...
    // NOWE: osobne pane dla narzędzi pomiaru
  ...
  async function setLayerVisible(rec, visible) {
    ...
    if (visible && !has)  layer.addTo(api._map);
    else if (!visible && has) api._map.removeLayer(layer);
  }
  ...
  async function setLayerOpacity(rec, opacity) {
    ...
    if (Number.isFinite(o) && typeof layer.setOpacity === 'function') layer.setOpacity(o);
  }
  ...
  async function rebuildBaseAndOverlays(rebuildOpts = {}) {
    ...
    // Capture only when the caller has not already snapshotted the old view.
    if (preserveView && !savedViewReady) viewManager.saveView();

    const baseId = api._lockedBaseId || api.baseCanvasId();
    if (!baseId) { return; }

    // Remove only IIIF layers
    const toRemove = [];
    api._map.eachLayer((l) => {
      const p = l?.options?.pane;
      if (p === 'iiif-base' || p === 'iiif-overlays') toRemove.push(l);
    });
    toRemove.forEach((l) => { try { if (api._map.hasLayer(l)) api._map.removeLayer(l); } catch (_) {} });
  ...
  api.init = async (containerEl) => {
    ...
    try {
      ...
      api._map.on('click', (e) => {
        ...
        if (!rec) return;
        const maxZ = getIiifLayerMaxZoom(rec?._layer);
        console.log(LOG, 'IIIF layer maxZoom:', maxZ);
        ...
        onMapClick?.({ s: maxZ, x: xc, y: yc, width: w, height: h, baseCanvasId: baseId, canvas: rec.canvas, originalEvent: e.originalEvent });
      });
  ...
  api.setManifest = async (manifest, manifestOpts = {}) => {
    ...
    // Snapshot NOW, while old canvases still exist in index
    if (preserveView) viewManager.saveView();

    function inGroup(canvas) {
      const dem     = isDemCanvas(canvas);
      const prod    = isDemProductCanvas(canvas);
      const demFam  = dem || prod;
      const include = api.group() === 'dem' ? demFam : !demFam;
      return include;
    }
    ...
    (canvases.length ? canvases : all).forEach((canvas, idx) => {
      ...
      api._canvasIndex.set(id, { id, canvas, label, serviceUrl, w, h, _layer: null, isBase: false });
    });

    const pickerOpts = Array.from(api._canvasIndex.values()).map((rec) => ({ id: rec.id, label: rec.label }));
    api.canvasOptions(pickerOpts);

    const current = api.baseCanvasId();
    if (current && api._canvasIndex.has(current)) {
      // dbg('setManifest: keeping base=', current);
    } else {
      const src     = canvases.length ? canvases : all;
      const best    = pickLargestCanvas(src);
      const bestId  = best?.id || best?.['@id'] || pickerOpts[0]?.id || null;
      // dbg('setManifest: new base=', bestId);
    ...
    api._subscriptions.push(api.baseCanvasId.subscribe((newId) => {
      // dbg('baseCanvasId →', newId);
      rebuildBaseAndOverlays({ preserveView: false }).catch((e) => setError(String(e?.message || e)));
    }));

    if (api._map) {
      await rebuildBaseAndOverlays({ preserveView, savedViewReady: preserveView });
    }
  };
  ...
  api.setGroup = async (group, groupOpts = {}) => {
    ...
    api.group(newGroup);
    if (api._manifest) {
      await api.setManifest(api._manifest, { preserveView: true, ...groupOpts });
    }
  };
  ...
  api.getViewportCenterInfo = () => {
    ...
    if (!crs?.latLngToPoint || typeof map.getSize !== 'function' || typeof map.containerPointToLatLng !== 'function') {
      return null;
    }
    ...
    if (!pixelAtZoom0 || !Number.isFinite(pixelAtZoom0.x) || !Number.isFinite(pixelAtZoom0.y)) {
      return null;
    }

    const maxZ = api.getCanvasMaxZoom(baseId) ?? 0;
  ...
  api.getCanvasMaxZoom = (canvasId) => {
    ...
    const maxDim = Math.max(Number(rec.w || 0), Number(rec.h || 0));
  ...
  api.dispose = () => {
    disposeSubs();
    try { if (api._map) api._map.remove(); } catch (_) {}
    api._map        = null;
    api._container  = null;
    api._manifest   = null;
    api._canvasIndex.clear();
